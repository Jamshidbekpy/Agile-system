...
class TaskAssignAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        if task.creator != creator:
            return Response({"error": _("You are not the creator of this task")}, status=403)  
        serializer = TaskAssignSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save(task=task)
            return Response({"success": _(f"User {serializer.validated_data['assignee']} was assigned to task {task.title}.")})
