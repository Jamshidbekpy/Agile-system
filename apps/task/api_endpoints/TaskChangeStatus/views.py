...
class TaskChangeStatusAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        if (
            task.status in [TaskStatus.BACKLOG, TaskStatus.REJECTED, TaskStatus.DONE]
            and request.user.role == "developer"
        ):
            return Response(
                {"error": _("You can't change the status of this task")}, status=403
            )
        ...
        if (
            new_status
            in [TaskStatus.BACKLOG, TaskStatus, TaskStatus.REJECTED, TaskStatus.DONE]
            and request.user.role == "developer"
        ):
            return Response(
                {"error": _("You can't change the status of this task")}, status=403
